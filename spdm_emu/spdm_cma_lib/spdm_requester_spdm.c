...
libspdm_return_t spdm_device_send_message(spdm_conn_t *spdm_conn,
                                          size_t request_size, const void *request,
                                          uint64_t timeout)
{
    bool result;

    result = send_platform_data(spdm_conn->m_socket, SOCKET_SPDM_COMMAND_NORMAL,
...
libspdm_return_t spdm_device_receive_message(spdm_conn_t *spdm_conn,
                                             ...
                                             uint64_t timeout)
{
    ...
    result = receive_platform_data(spdm_conn->m_socket, &command, *response,
                                   response_size);
    if (!result) {
        printf("receive_platform_data Error - %x\n",
#ifdef _MSC_VER
 ...
 **/
libspdm_return_t pci_doe_send_receive_data(spdm_conn_t *spdm_conn,
                                           size_t request_size, const void *request,
                                           size_t *response_size, void *response)
{
    ...
    result = communicate_platform_data(
        spdm_conn->m_socket, SOCKET_SPDM_COMMAND_NORMAL,
